export const createOffer = async (connection, localStream, userToCall, doOffer, database, username) => {
  ...
  export const initiateLocalStream = async () => {
    try {
        // Promise, we need to call getuserMedia() to request user media
  ...
  export const initiateConnection = async () => {
    try {
      ...
      //STUN server enables clients to find out their public IP address, 
      //NAT type, and the Internet-facing port associated by the NAT device 
      //with a particular local port. This information is used to set up a UDP
      //communication session between the client and the VoIP provider to establish a call
      const configuration={
          iceServers: [{urls: 'stun:stun2.1.google.com:9302'}]
      }
  ...
  export const addCandidate = (conn, notif) => {
    // apply the n ew received candidate to the connection
    // get candidate
